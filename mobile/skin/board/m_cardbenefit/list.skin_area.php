            ...
            ?>
                ...
                <li class="add_btn">
                    <a href="<?php echo "/bbs/board.php?bo_table=".$bo_table."&wr_id=".$list[$i]['wr_id']."&me_code=".$me_code."&info=benefit&num=".$num; ?><?=lang_url_a($_SESSION['lang'])?>" class='link1' id="div_<?=$list[$i]['wr_id']?>"  >
					 ...
					 <div onclick="fnMove('1')" class="    info info_focus_area_m info_num_<?=$cnt?>"  valLink='/bbs/board.php?bo_table=cardbenefit&wr_id=<?=$list[$i]['wr_id']?>&info=benefit&me_code=30&num=2&lang=<?=lang_url($_SESSION['lang'])?>'  valInfoLat='<?=$list[$i]['wr_lat']?>' valCnt='<?=$cnt?>' valId='<?=$list[$i]['wr_id']?>' valInfoLng='<?=$list[$i]['wr_lng']?>'  valInfoNameko='<?=$list[$i]['wr_subject_ko_KR']?>' valInfoNameen='<?=$list[$i]['wr_subject_en_US']?>' valInfoNameja='<?=$list[$i]['wr_subject_ja_JP']?>' valInfoNamech1='<?=$list[$i]['wr_subject_zh_CN']?>' valInfoNamech2='<?=$list[$i]['wr_subject_zh_TW']?>'  valdetail='<?=_t('자세히보기')?>' ><img src="http://www.koreatourcard.kr/img/sub/sub3/sub3_3/ktc_map_icon_<?=$cnt?>.png">
                ...
                <div class='map_info' valSrc='<?php echo $thumb['src']?>'  vallocal1='<?=_t($list[$i]['wr_local1_ko_KR'])?>' valLink='/bbs/board.php?bo_table=cardbenefit&wr_id=<?=$list[$i]['wr_id']?>&info=benefit&me_code=30&num=2&lang=<?=lang_url($_SESSION['lang'])?>' valTotalpage='<?=$totalPage?>' valId='<?=$list[$i]['wr_id']?>' valInfoNameko='<?=$list[$i]['wr_subject_ko_KR']?>' valInfoNameen='<?=$list[$i]['wr_subject_en_US']?>' valInfoNameja='<?=$list[$i]['wr_subject_ja_JP']?>' valInfoNamech1='<?=$list[$i]['wr_subject_zh_CN']?>' valInfoNamech2='<?=$list[$i]['wr_subject_zh_TW']?>' valInfoAddressko='<?=$list[$i]['wr_subject_ko_KR']?>' valInfoAddressen='<?=$list[$i]['info_address_en_US']?>' valInfoAddressja='<?=$list[$i]['info_address_ja_JP']?>' valInfoAddressch1='<?=$list[$i]['info_address_zh_CN']?>' valInfoAddressch2='<?=$list[$i]['info_address_zh_TW']?>' valInfoLat='<?=$list[$i]['wr_lat']?>' valInfoLng='<?=$list[$i]['wr_lng']?>' valInfoServicetimeko='<?=$list[$i]['wr_subject_ko_KR']?>' valInfoServicetimeen='<?=$list[$i]['info_servicetime_en_US']?>' valInfoServicetimeja='<?=$list[$i]['info_servicetime_ja_JP']?>' valInfoServicetimech1='<?=$list[$i]['info_servicetime_zh_CN']?>' valInfoServicetimech2='<?=$list[$i]['info_servicetime_zh_TW']?>' valNameko='<?=$list[$i]['wr_subject_ko_KR']?>' valNameen='<?=$list[$i]['name_en_US']?>' valNameja='<?=$list[$i]['name_ja_JP']?>' valNamech1='<?=$list[$i]['name_zh_CN']?>' valNamech2='<?=$list[$i]['name_zh_TW']?>' valTypeko='<?=$list[$i]['wr_subject_ko_KR']?>' valTypeen='<?=$list[$i]['type_en_US']?>' valTypeja='<?=$list[$i]['type_ja_JP']?>' valTypech1='<?=$list[$i]['type_zh_CN']?>' valTypech2='<?=$list[$i]['type_zh_TW']?>'  valdetail='<?=_t('자세히보기')?>'></div>
