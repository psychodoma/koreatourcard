...
$(function(){
	...
	$('.info_focus_area').click(function(){
		...
		$('.info_focus_area').each(function(index){
			if(cnt == index){
				$(this).children('img').attr('src','http://www.koreatourcard.kr/img/sub/sub3/sub3_3/ktc_map_icon_red_'+cnt+'.png');
			}else{
				$(this).children('img').attr('src','http://www.koreatourcard.kr/img/sub/sub3/sub3_3/ktc_map_icon_'+index+'.png');
			}
...
<div class="sub1_3area">
                    ...
                    <ul class="sub2_1form1">
                        <li class="">
                            <select class="Address_D" name='word' id="color" title="select color">
                                <option class='base_select' value=''><?=_t('지역을 선택하세요')?></option>
			...
			<?if( $_SESSION['lang'] != "en_US" ){?>
				<h2><?=_t('총')?> <span><?php echo number_format($total_count)?></span><?=_t('개')?> <?=_t('브랜드의 혜택이 있습니다.')?></h2>
			<?}else{?>
				<h2>Benefits of <span><?php echo number_format($total_count)?></span> brands are available </h2>
			<?}?>
            ...
            ?>
                ...
                <li class="add_btn">
                    <a href="<?php echo "/bbs/board.php?bo_table=".$bo_table."&wr_id=".$list[$i]['wr_id']."&me_code=".$me_code."&info=benefit&num=".$num; ?><?=lang_url_a($_SESSION['lang'])?>" class='link1' id="div_<?=$list[$i]['wr_id']?>"  >
					 ...
					 <div onclick="fnMove('1')" class="    info info_focus_area info_num_<?=$cnt?>"  valLink='/bbs/board.php?bo_table=cardbenefit&wr_id=<?=$list[$i]['wr_id']?>&info=benefit&me_code=30&num=2&lang=kr'  valInfoLat='<?=$list[$i]['wr_lat']?>' valCnt='<?=$cnt?>' valId='<?=$list[$i]['wr_id']?>' valInfoLng='<?=$list[$i]['wr_lng']?>'  valInfoNameko='<?=$list[$i]['wr_subject_ko_KR']?>' valInfoNameen='<?=$list[$i]['wr_subject_en_US']?>' valInfoNameja='<?=$list[$i]['wr_subject_ja_JP']?>' valInfoNamech1='<?=$list[$i]['wr_subject_zh_CN']?>' valInfoNamech2='<?=$list[$i]['wr_subject_zh_TW']?>'  valdetail='<?=_t('자세히보기')?>' ><img src="http://www.koreatourcard.kr/img/sub/sub3/sub3_3/ktc_map_icon_<?=$cnt?>.png">
