...
include_once(G5_LIB_PATH.'/thumbnail.lib.php');
		...
		if($_SESSION['lang']=="ko_KR"){
			//$result_add = sql_query(" select * from g5_write_".$bo_table." where wr_1 = 'on' and wr_6 = '' order by wr_num limit ".($page*$page_rows).", ".$page_rows);
			$reuslt_cnt = sql_fetch("select count(*) cnt from ( select * from g5_write_".$bo_table." where wr_1 = 'on' and wr_6 = '' order by wr_num limit ".($page*$page_rows).", ".$page_rows.") a" );
			//$reuslt_total = sql_fetch(" select count(*) cnt from g5_write_".$bo_table." where wr_1 = 'on' and wr_6 = '' ");
		...
		$m_sql_result[0] = str_replace("\'on\'", "'on'" ,$m_sql_result[0]);
		$m_sql_result[0] = str_replace("\'\'", "''" ,$m_sql_result[0]);

		if($m_sql_result[1] == "0, 5"){
			$result_add = sql_query($m_sql_result[0]."limit 0, ".$page);
			$m_sql = $m_sql_result[0]."limit 0, ".$page;
		}else{
			$result_add = sql_query($m_sql_result[0]."limit ".($page*$page_rows).", ".$page_rows);
			$m_sql = $m_sql_result[0]."limit ".($page*$page_rows).", ".$page_rows;
		}

		//$result_add = sql_query($m_sql_result[0]);

		$reuslt_total['cnt'] = $m_sql_total;
...
$(function(){
    $('.subMore_btn_area a').click(function(){
        ...
        $.ajax({
            url: "/bbs/ajax.listadd_knotice.php",
            data: {
