...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    if (file_exists($bimg))
    {
        ...
        if ($row['bn_url'][0] == '#')
            ...
            $banner .= '<a href="'.G5_URL.'/bannerhit.php?bn_id='.$row['bn_id'].'&amp;url='.urlencode( $row[$blinke_URL]).'"'.$bn_new_win.' style="width:170px;height:56px;display:block;background:rgba(255,255,255,0)" >';
        }

		echo $banner.'<img src="'.$bimg_URL.'" alt="'.$row['bn_alt'].'" style="padding:5px 10px;" width="'.$size[0].'" height="'.$size[1].'"'.$bn_border.'>';
