...
add_stylesheet('<link rel="stylesheet" href="'.G5_THEME_URL.'/skin/banner'.'/style.css">', 10);
?>
...
{
    if ($i==0) echo '<section id="sbn_idx_'.$position.'" class="sbn">'.PHP_EOL.'<h2>'._t('쇼핑분류배너영역').'</h2>'.PHP_EOL.'<ul>'.PHP_EOL;
    //print_r2($row);
    ...
    if (file_exists($bimg))
    {
		...
		*/
        ...
        echo $banner.'<img src="'.$bimg_URL.'" width="'.$max_width.'" height="'.$max_height.'" alt="'._t($row['bn_alt']).'"'.$bn_border.'>';
...
$(function() {
    $("#sbn_idx_<?=$position?>").bannerRolling();
