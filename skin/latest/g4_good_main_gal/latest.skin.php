...
if(!$img_width) $img_width = 512;
if(!$img_height) $img_height = 132;
if(!$slide_control_height) $slide_control_height = 36; /// default
?>
<style>
a img {
border : 0;
...
<div id="m_header"><div class="m_wrap">
 <div id="slide-holder">
	...
	?>
	<a href="<?php echo set_http($list[$i][wr_link1])?>" style='color:#fff;border:0px;' target='_blank'><img id="slide-img-<?php echo ($i+1)?>" src="<?php echo $imagepath?>" width="<?php echo $img_width?>" height="<?php echo $img_height?>" alt="<?php echo _t($list[$i][wr_subject])?>" /></a>
	<?php } ?>
 ...
 <script type="text/javascript">
	...
	<?php for ($i=0; $i<count($list); $i++) { ?>
	{"id":"slide-img-<?php echo ($i+1)?>","client":"<?php echo _t($list[$i][subject])?>","desc":""}<?if(($i+1) != count($list)){?>,<?php } ?>
	<?php } ?>
	];
 </script>
</div></div>
