...
if($board[bo_4] and $board[bo_5]) {
   $image_width = $board[bo_4] - 8;
   $image_height = $board[bo_5] - 8;
   $image_div_width = $image_width;
} else {
   $image_width = $config2w[cf_width_main] - 8;
   $image_height = intval($image_width / 3) - 8;
   $image_div_width = $image_width;
}
...
/// old compatible
if(!$img_width) $img_width  = $image_width;
if(!$img_height) $img_height = $image_height;
?>
<!-- 최신글용 스타일시트 -->
<link href="<?php echo $latest_skin_url?>/style.css" rel="stylesheet" type="text/css">

<!-- 최신글 시작 -->
<div class="latestThumb4">
<div class="sideTable">
    <?php if(0) { ?>
    <div class="sideTitle" style="height:25px; margin-bottom:-50px">
...
?>
	...
	<center><div class="latestIMG">
		...
		<?php if(preg_match("/\.(swf|wmv|asf|flv)$/i", $img_src)) { ?>
		<script>doc_write(flash_movie('<?php echo $img_src?>', 'flash<?php echo $i?>', '<?php echo $image_width?>', '<?php echo $image_height?>', 'transparent'));</script>
		<?php } else { ?>
		<img src="<?php echo $img_src?>" width="<?php echo $image_width?>" border="0">
		<?php } ?>
