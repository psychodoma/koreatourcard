...
if($_SESSION['lang'] == "ko_KR"){
	$next_page_info_result = sql_fetch("select * from g5_write_".$_GET['bo_table']."  where wr_1 = 'on' and wr_id < ".$wr_id." order by wr_id desc");
	$prev_page_info_result = sql_fetch("select * from g5_write_".$_GET['bo_table']."  where wr_1 = 'on' and wr_id > ".$wr_id." ");
}else if($_SESSION['lang'] == "en_US"){
	$next_page_info_result = sql_fetch("select * from g5_write_".$_GET['bo_table']."  where wr_2 = 'on' and wr_id < ".$wr_id." order by wr_id desc");
	$prev_page_info_result = sql_fetch("select * from g5_write_".$_GET['bo_table']."  where wr_2 = 'on' and wr_id > ".$wr_id." ");
}else if($_SESSION['lang'] == "ja_JP"){
	$next_page_info_result = sql_fetch("select * from g5_write_".$_GET['bo_table']."  where wr_3 = 'on' and wr_id < ".$wr_id." order by wr_id desc");
	$prev_page_info_result = sql_fetch("select * from g5_write_".$_GET['bo_table']."  where wr_3 = 'on' and wr_id > ".$wr_id." ");
}else if($_SESSION['lang'] == "zh_CN"){
	$next_page_info_result = sql_fetch("select * from g5_write_".$_GET['bo_table']."  where wr_4 = 'on' and wr_id < ".$wr_id." order by wr_id desc");
	$prev_page_info_result = sql_fetch("select * from g5_write_".$_GET['bo_table']."  where wr_4 = 'on' and wr_id > ".$wr_id." ");
}else if($_SESSION['lang'] == "zh_TW"){
	$next_page_info_result = sql_fetch("select * from g5_write_".$_GET['bo_table']."  where wr_5 = 'on' and wr_id < ".$wr_id." order by wr_id desc");
	$prev_page_info_result = sql_fetch("select * from g5_write_".$_GET['bo_table']."  where wr_5 = 'on' and wr_id > ".$wr_id." ");
}else{
	$next_page_info_result = sql_fetch("select * from g5_write_".$_GET['bo_table']."  where wr_1 = 'on' and wr_id < ".$wr_id." order by wr_id desc");
	$prev_page_info_result = sql_fetch("select * from g5_write_".$_GET['bo_table']."  where wr_1 = 'on' and wr_id > ".$wr_id." ");
}
				...
				<tr>
					<td class="title">
						...
						<p>
                            <?php
                                if ($category_name) echo $view['ca_name'].' | '; // 분류 출력 끝
                                echo substr(cut_str(get_text($view['wr_last']), 70),0,10); // 글제목 출력
                            ?>
                        </p>
					</td>
				</tr>
