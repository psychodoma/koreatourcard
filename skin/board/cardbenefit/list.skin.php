...
?>
		...
		<div class="sub3_2center">
			...
			<table width="100%" class="sub32_table">
				<tr>
					<td colspan="3" class="sub32_tb_head"><h2 class="sub32_tableHead" style='float:left; line-height:30px;'>
                         ...
                         <form name="fsearch" method="get" class="sub_search" style="margin-bottom:0;" onsubmit="return fsearchbox_submit(this);">
                            <fieldset>
                                <ul>
                                    ...
                                    <li class="btn">
                                        <input type="submit" value="<?=_t('검색')?>" class="btn_submit" title="<?=_t('검색')?>">
                                    </li>
            ...
            function fsearchbox_submit(f)
            {
				...
				<tr>
					...
					<td class="sub32_tb_logo" style='position:relative;'>
						...
						<p><a href="<?php echo $list[$i]['href']."&info=benefit&me_code=".$me_code."&num=".$num.lang_url_a($_SESSION['lang']); ?>" title="<?php echo $list[$i]['wr_subject'.$lang]?>"><?=_t('자세히보기')?> <span>></span></a></p>
					</td>
					<!-- <td class="sub32_tb_benti"><?=_t('혜택')?></td> -->
					...
					<? $sql_bene = sql_query("select * from g5_board_file where wr_id =".$list[$i]['wr_id']." order by bf_no"); ?>
						<ul>
							...
							?>
								...
								<li>
									<?if($_SESSION['lang'] == "ko_KR"){
										echo $bene_reulst['bf_content'];
									}else{
										echo $bene_reulst['bf_content_'.$_SESSION['lang']];
									}
									?>
								</li>
							<?}}?>
...
$(function(){
	$('.sub32_tb_list li').each(function(index){
