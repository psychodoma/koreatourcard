		  ...
		  <div class="sub2_1form_area">
			...
			<ul class="sub3_1List">
                ...
                for($i=0; $i<count($list); $i++){
                    ...
                    <li class="info info_focus_area info_num_<?=$cnt?><?if($i%4 == 3) echo ' li_margin_r ';?>"  valLink='/bbs/board.php?bo_table=cardbenefit&wr_id=<?=$list[$i]['wr_id']?>&info=benefit&me_code=30&num=2&lang=kr'  valInfoLat='<?=$list[$i]['wr_lat']?>' valCnt='<?=$cnt?>' valId='<?=$list[$i]['wr_id']?>' valInfoLng='<?=$list[$i]['wr_lng']?>'  valInfoNameko='<?=$list[$i]['wr_subject_ko_KR']?>' valInfoNameen='<?=$list[$i]['wr_subject_en_US']?>' valInfoNameja='<?=$list[$i]['wr_subject_ja_JP']?>' valInfoNamech1='<?=$list[$i]['wr_subject_zh_CN']?>' valInfoNamech2='<?=$list[$i]['wr_subject_zh_TW']?>'  valdetail='<?=_t('자세히보기')?>'  >
						<div class="grid">
