...
<tr>
    <td style='word-break:break-all;'>
        ...
        if ($stx) 
        { 
            echo "<ul type=circle><li><b>"._t("검색된 게시판 리스트")."</b> (<b>{$board_count}</b>"._t("개의 게시판").", <b>".number_format($total_count)."</b>"._t("개의 게시글").", <b>".number_format($page)."/".number_format($total_page)."</b> "._t("페이지").")</ul>";
            if ($board_count)
            {
                echo "<ul><ul type=square style='line-height:130%;'>";
                if ($onetable)
                    echo "<li><a href='?$search_query&gr_id=$gr_id'>"._t("전체게시판 검색")."</a>";
                echo $str_board_list;
                echo "</ul></ul>";
            }
            else
            {
                echo "<ul style='line-height:130%;'><li>"._t("검색된 자료가 하나도 없습니다.")."</ul>";
            }
        }
        ?>
        ...
        { 
            echo "<ul type=circle><li><b><a href='./board.php?bo_table={$search_table[$idx]}&{$search_query}'><u>"._t($bo_subject[$idx])."</u></a>"._t("에서의 검색결과")."</b></ul>";
            $comment_href = "";
            for ($i=0; $i<count($list[$idx]) && $k<$rows; $i++, $k++) 
            {
                ...
                echo "</u></a> [<a href='{$list[$idx][$i][href]}{$comment_href}' target=_blank>"._t("새창")."</a>]<br>";
