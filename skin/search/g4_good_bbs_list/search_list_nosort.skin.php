...
<table width="98%" align="center" cellpadding="0" cellspacing="0"><tr><td>
    ...
    <tr>
        ...
        <th><?php echo _t('날짜'); ?></th>
        <th><?php echo _t('조회'); ?></th>
        <?php if ($is_good) { ?><th><?php echo _t('추천'); ?></th><?php }?>
        <?php if ($is_nogood) { ?><th><?php echo _t('비추천'); ?></th><?php }?>
    </tr>
    ...
    { 
        ...
        for ($i=0; $i<count($list[$idx]) && $k<$rows; $i++, $k++) 
        {
            ...
            echo "[";
            echo "<a href='$_SERVER[PHP_SELF]?$search_query&gr_id=$gr_id&srows=$srows&onetable={$search_table[$idx]}&sca=&page='>"._t($row2[bo_subject])."</a>";
            if($list[$idx][$i][ca_name]) 
                echo " > <a href='$_SERVER[PHP_SELF]?$search_query&gr_id=$gr_id&srows=$srows&onetable={$search_table[$idx]}&sca={$list[$idx][$i][ca_name]}&page='>{$list[$idx][$i][ca_name]}</a>";
            else
                echo "";
            echo "] ";
            ...
            if($thumb_use) {
                $thumb_src = $g5[path]."/data/file/".$search_table[$idx]."/thumb/".$list[$idx][$i][wr_id];
                $thumb_src_url = $g5[url]."/data/file/".$search_table[$idx]."/thumb/".$list[$idx][$i][wr_id];
