...
this.init = function() {
    ...
    ge_editor.write("body { padding:0px; margin:5px; font-size:10pt; font-family:NanumGothic; }");
    ge_editor.write("td { font-size:10pt; font-family:NanumGothic; }");
...
this.font_family = function(obj) {
    var font_kor  = ['Gulim', 'GulimChe', 'Dotum', 'DotumChe', 'Batang', 'BatangChe', 'Gungsuh', 'GungsuhChe', 'NanumGothic','NanumMyeongjo'];
    var font_kori = ['굴림', '굴림체', '돋움', '돋움체', '바탕', '바탕체', '궁서', '궁서체', '나눔고딕', '나눔명조'];
    ...
    info.innerHTML = "<b>글꼴</b> <a href=\"javascript:"+ge_name+".clear_option();\" style=\"color:#ccc;\">[닫기]</a>";
...
this.font_size = function(obj) {
    ...
    var info = document.createElement("div");
    info.style.fontSize = 12;
    info.innerHTML = "<b>크기</b> <a href=\"javascript:"+ge_name+".clear_option();\" style=\"color:#ccc;\">[닫기]</a>";
    div.appendChild(info);
...
this.insert_image = function(obj) {
    ...
    submit.onclick  = function() {
        if (ge_is_empty.value != 'true') {
            ...
            /// 상대 경로 구현. 아래 1 개 라인 추가
            file  = g5_url + file.substring( file.indexOf('/data/geditor/') );
...
this.insert_emoticon = function(obj) {
    ...
    for (var i=1; i<=ge_emoticon_count; i++) {
        ...
        /// 상대 경로 구현. 아래 3 개 라인으로 교체
        var file = img.src;
        file  = g5_url + file.substring( file.indexOf('/geditor/emoticons/') );
        img.onclick = new Function(ge_name+".insert_emoticon_to_editor(\""+file+"\")");
