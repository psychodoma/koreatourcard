...
class GUMP {
     ...
     * @return string
     */
    public function get_readable_errors($convert_to_string = false, $field_class = 'gump-field tag tag-warning', $error_class = 'gump-error-message alert alert-danger') {
...
/*
  Validater 추가
 */
GUMP::add_validator("tel_number", function($field, $input, $param = NULL) {
    if (!isset($input[$field]) || empty($input[$field])) {
        return;
    }

    $regex = '/^01(?:0|1|[6-9])-(?:\d{3}|\d{4})-\d{4}$/i';
