...
function get_list_thumbnail($bo_table, $wr_id, $thumb_width, $thumb_height, $is_create=false, $is_crop=true, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3', $option=0)
{
    ...
    $sql = " select bf_file, bf_content,bf_width,bf_height,bf_ck from {$g5['board_file_table']}
    ...
    $thumb = array("src"=>$src, "ori"=>$ori, "alt"=>$alt, "width"=>$row['bf_width'], "height"=>$row['bf_height']);

    return $thumb;
}

function get_list_thumbnail_ktc($bo_table, $wr_id, $thumb_width, $thumb_height, $is_create=false, $is_crop=true, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3', $option=0)
{
    global $g5, $config;
    $filename = $alt = "";
    $edt = false;
    ...
    $sql = " select bf_file, bf_content,bf_width,bf_height,bf_ck from {$g5['board_file_table']}
                where bo_table = '$bo_table' and wr_id = '$wr_id' and bf_type between '0' and '3' order by bf_no limit $option, 1 ";
...
function get_view_thumbnail($contents, $thumb_width=0)
{
    ...
    for($i=0; $i<count($matches[1]); $i++) {
        ...
        if(is_file($srcfile)) {
            ...
            if ($width) {
                $thumb_tag = '<img src="'.G5_URL.str_replace($filename, $thumb_file, $data_path).'"  width="'.$width.'" height="'.$height.'"/>';
            } else {
                $thumb_tag = '<img src="'.G5_URL.str_replace($filename, $thumb_file, $data_path).'" />';
            }
