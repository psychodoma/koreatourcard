...
function get_file($bo_table, $wr_id)
{
    ...
    while ($row = sql_fetch_array($result))
    {
        ...
        $file[$no]['bf_ck'] = $row['bf_ck'];

        $file[$no]['bf_content_en_US'] = $row['bf_content_en_US'];
        $file[$no]['bf_content_ja_JP'] = $row['bf_content_ja_JP'];
        $file[$no]['bf_content_zh_CN'] = $row['bf_content_zh_CN'];
        $file[$no]['bf_content_zh_TW'] = $row['bf_content_zh_TW'];  

        $file[$no]['content_en_US'] = get_text($row['bf_content_en_US']);
        $file[$no]['content_ja_JP'] = get_text($row['bf_content_ja_JP']);
        $file[$no]['content_zh_CN'] = get_text($row['bf_content_zh_CN']);
        $file[$no]['content_zh_TW'] = get_text($row['bf_content_zh_TW']);  
...
function get_list($write_row, $board, $skin_url, $subject_len=40, $BBS_PATH = G5_BBS_URL) 
{
    ...
    if ($board['bo_use_list_content'])
	{
        ...
        ///$list['content'] = conv_content(_t($list['wr_content']), $html);
	}
    ...
    $list['icon_reply'] = '';
    if ($list['reply'])
        $list['icon_reply'] = '<img src="'.$skin_url.'/img/icon_reply.gif" style="margin-left:'.$list['reply'].'px;" alt="'._t('답변글').'">';

    $list['icon_link'] = '';
    if ($list['wr_link1'] || $list['wr_link2'])
        $list['icon_link'] = '<img src="'.$skin_url.'/img/icon_link.gif" alt="'._t('관련링크').'">';
    ...
    if ($board['bo_hot'] && $list['wr_hit'] >= $board['bo_hot'])
        $list['icon_hot'] = '<img src="'.$skin_url.'/img/icon_hot.gif" alt="'._t('인기글').'">';

    $list['icon_secret'] = '';
    if (strstr($list['wr_option'], 'secret'))
        $list['icon_secret'] = '<img src="'.$skin_url.'/img/icon_secret.gif" alt="'._t('비밀글').'">';
    ...
    if ($list['file']['count'])
        $list['icon_file'] = '<img src="'.$skin_url.'/img/icon_file.gif" alt="'._t('첨부파일').'">';

    return $list;
}
function get_list_ktc($write_row, $board, $skin_url, $subject_len=40, $BBS_PATH = G5_BBS_URL) 
{
    global $g5, $config;
    global $qstr, $page;
    ...
    $list['icon_new'] = '';
    if ($board['bo_new'] && $list['wr_datetime'] >= date("Y-m-d H:i:s", G5_SERVER_TIME - ($board['bo_new'] * 3600)))
        $list['icon_new'] = '<img style="position: absolute; left:0; top:0;" src="/img/new.png" alt="'._t('새글').'">';
...
function get_category_option_ktc($bo_table='', $ca_name='')
{
    global $g5, $board, $is_admin;
...
function get_sideview($mb_id, $name='', $email='', $homepage='')
{
    ...
    if ($mb_id) {
        ...
        $tmp_name = '<a href="'.G5_BBS_URL.'/profile.php?mb_id='.$mb_id.'" class="sv_member" title="'.$name.' '._t('자기소개').'" target="_blank" onclick="return false;">';
        ...
        $tmp_name = '<a href="'.G5_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;sca='.$sca.'&amp;sfl=wr_name,1&amp;stx='.$name.'" title="'.$name.' '._t('이름으로 검색').'" class="sv_guest" onclick="return false;">'.$name.'</a>';
        $title_mb_id = '['._t('비회원').']';
    }
    ...
    $str2 = "<span class=\"sv\">\n";
    if($mb_id)
        $str2 .= "<a href=\"".G5_BBS_URL."/memo_form.php?me_recv_mb_id=".$mb_id."\" onclick=\"win_memo(this.href); return false;\">"._t("쪽지보내기")."</a>\n";
    if($email)
        $str2 .= "<a href=\"".G5_BBS_URL."/formmail.php?mb_id=".$mb_id."&amp;name=".urlencode($name)."&amp;email=".$email."\" onclick=\"win_email(this.href); return false;\">"._t("메일보내기")."</a>\n";
    if($homepage)
        $str2 .= "<a href=\"".$homepage."\" target=\"_blank\">"._t("홈페이지")."</a>\n";
    if($mb_id)
        $str2 .= "<a href=\"".G5_BBS_URL."/profile.php?mb_id=".$mb_id."\" onclick=\"win_profile(this.href); return false;\">"._t("자기소개")."</a>\n";
    if($bo_table) {
        if($mb_id)
            $str2 .= "<a href=\"".G5_BBS_URL."/board.php?bo_table=".$bo_table."&amp;sca=".$sca."&amp;sfl=mb_id,1&amp;stx=".$mb_id."\">"._t("아이디로 검색")."</a>\n";
        else
            $str2 .= "<a href=\"".G5_BBS_URL."/board.php?bo_table=".$bo_table."&amp;sca=".$sca."&amp;sfl=wr_name,1&amp;stx=".$name."\">"._t("이름으로 검색")."</a>\n";
    }
    if($mb_id)
        $str2 .= "<a href=\"".G5_BBS_URL."/new.php?mb_id=".$mb_id."\">"._t("전체게시물")."</a>\n";
    if($is_admin == "super" && $mb_id) {
        $str2 .= "<a href=\"".G5_ADMIN_URL."/member_form.php?w=u&amp;mb_id=".$mb_id."\" target=\"_blank\">"._t("회원정보변경")."</a>\n";
        $str2 .= "<a href=\"".G5_ADMIN_URL."/point_list.php?sfl=mb_id&amp;stx=".$mb_id."\" target=\"_blank\">"._t("포인트내역")."</a>\n";
    }
    $str2 .= "</span>\n";
    $str .= $str2;
...
{
    $arr_yoil = array (_t('일'), _t('월'), _t('화'), _t('수'), _t('목'), _t('금'), _t('토'));
...
function bad_tag_convert($code)
{
    ...
    if ($is_admin && $member['mb_id'] != $view['mb_id']) {
        ...
        $code = preg_replace_callback("#(\<(embed|object)[^\>]*)\>?(\<\/(embed|object)\>)?#i",
                    create_function('$matches', 'return "<div class=\"embedx\">'._t('보안문제로 인하여 관리자 아이디로는 embed 또는 object 태그를 볼 수 없습니다. 확인하시려면 관리권한이 없는 다른 아이디로 접속하세요.').'</div>";'),
                    $code);
    }
...
function check_device($device)
{
    ...
    if ($device=='pc' && G5_IS_MOBILE) {
        alert(_t('PC 전용 게시판입니다.'), G5_URL);
    } else if ($device=='mobile' && !G5_IS_MOBILE) {
        alert(_t('모바일 전용 게시판입니다.'), G5_URL);
    }
}
...
function get_skin_path($dir, $skin)
{
    ...
    if(preg_match('#^theme/(.+)$#', $skin, $match)) { // 테마에 포함된 스킨이라면
        ...
        if(G5_IS_MOBILE) {
            ...
            $skin_path = G5_THEME_MOBILE_PATH.'/'.G5_SKIN_DIR.'/'.$dir.'/'.$match[1];
            if(!is_dir($skin_path))
...
// 혜택 분류 카테고리 Num 받아오기
function caNameShift($ca_name){
		$sql = "select * from card_category where name_en_US = '$ca_name' ";
