...
if ($w == '' || $w == 'r') {
    ...
    $sql = " insert into $write_table
                set wr_num = '$wr_num',
                     ...
                     wr_simple_ko_KR = '$wr_simple_ko_KR',
                     wr_simple_en_US = '$wr_simple_en_US',
                     wr_simple_ja_JP = '$wr_simple_ja_JP',
                     wr_simple_zh_CN = '$wr_simple_zh_CN',
                     wr_simple_zh_TW = '$wr_simple_zh_TW',
    ...
    for ($i=0; $i<count($wr_course); $i++){
        $j = $i+1;
        $sql_course = " update g5_write_course
                    set
                        wr_course = '{$wr_id}',
                        wr_course_subject = '{$wr_course_subject[$i]}',
                        wr_cnt = '{$j}'
                        where wr_id = '{$wr_course[$i]}' 
                    ";
        sql_query($sql_course);
    }
    ...
    $del_query = sql_query(" select * from g5_write_course where wr_course =  ".$wr_id);



    while ( $row = sql_fetch_array($del_query) ){
        $sql_course = " update g5_write_course
                    set
                        wr_course = NULL,
                        wr_course_subject = NULL,
                        wr_cnt = NULL
